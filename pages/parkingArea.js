...
export default function ParkingArea() {
  ...
  const { parkingAreaFloor, parkingSlotNames, parkingAreaSlots, parkingAreaName, parkingAreaAddress,parkingAreaStatus,setParkingAreaStatus,calendarRestriction,setParkingAreaStatusToggle,rows,setRows,setParkingSlotNames,setParkingAreaFloor } = useGlobalContext();
    ...
    fetch(
      ...
      .then((response) => {
        ...
        const result = Object.values(
          response.ParkingAreas.reduce((acc, curr) => {
            const key = curr.parkingAreaName;
            if (!acc[key]) {
              acc[key] = {
                parkingAreaName: key,
                parkingAreaFloors: []
            ...
            if (!acc[key].parkingAreaFloors.includes(curr.parkingAreaFloor)) {
              acc[key].parkingAreaFloors.push(curr.parkingAreaFloor);
            }
            return acc;
          }, {})
        ).map(({ parkingAreaFloors }) => parkingAreaFloors);
        setParkingAreaFloor(result);

        // const groupedData = response.ParkingAreas.reduce((result, item) => {
        //   if (!result[item.parkingAreaName]) {
        //     result[item.parkingAreaName] = {};
        //   }
        //   if (!result[item.parkingAreaName][item.parkingAreaFloor]) {
        //     result[item.parkingAreaName][item.parkingAreaFloor] = [];
        //   }
        //   result[item.parkingAreaName][item.parkingAreaFloor].push(item.parkingFloorSlotNames);
        //   return result;
        // }, {});
        // // console.log(groupedData);
        // const finalData = Object.values(groupedData).map(floor => 
        //   Object.values(floor).map(slot => 
        //     [slot])
        //   );
        // setParkingSlotNames(finalData);
        ...
        const finalData = Object.entries(groupedData).map(([areaName, area]) => 
          Object.entries(area).map(([floor, slots]) => 
            slots)
          );
        setParkingSlotNames(finalData);
      })
